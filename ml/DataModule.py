...
class DataModule_hc2d(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        for i in range(len(theta)):
            self.theta.append(theta[i](x_Q))
            self.f.append(f[i](x_Q))
            self.etab.append(etab[i](x_Q))
            self.etat.append(etat[i](x_Q))
            indices = np.linspace(0,x.shape[1]-1, x.shape[1], dtype=int)
            indices_output = np.random.choice(indices, size=self.hyperparams['Q_L'], replace=False)
            self.x.append(x[i,indices_output])
            self.u.append(u[i,indices_output])
        ...
        self.f = torch.tensor(self.f, dtype=self.hyperparams['dtype']).reshape(len(theta),self.hyperparams['Q'],self.hyperparams['Q'])
        self.etab = torch.tensor(self.etab, dtype=self.hyperparams['dtype']).reshape(len(theta),self.hyperparams['Q'],self.hyperparams['Q'])
        self.etat = torch.tensor(self.etat, dtype=self.hyperparams['dtype']).reshape(len(theta),self.hyperparams['Q'],self.hyperparams['Q'])
